...
def main(params):
    ...
    encoder = (
        TransformerModel(model_params, dico, is_encoder=True, with_output=True)
        .cuda()
        .eval()
    )
    decoder = (
        TransformerModel(model_params, dico, is_encoder=False, with_output=True)
        .cuda()
        .eval()
    )
    ...
    for i in range(0, len(src_sent), params.batch_size):
        ...
        batch = torch.LongTensor(lengths.max().item(), lengths.size(0)).fill_(
            params.pad_index
        )
...
if __name__ == "__main__":
    ...
    assert (
        params.src_lang != ""
        and params.tgt_lang != ""
        and params.src_lang != params.tgt_lang
    )
