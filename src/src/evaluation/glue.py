...
class GLUE:
    ...
    def train(self):
        ...
        while True:
            ...
            # loss
            output = self.proj(
                self.embedder.get_embeddings(x, lengths, positions=None, langs=None)
            )
            if self.is_classif:
            ...
            if ns != 0 and ns % (10 * bs) < bs:
                logger.info(
                    ...
                    % (
                        ...
                        ns,
                        nw / (time.time() - t),
                        "XE" if self.is_classif else "MSE",
    ...
    def eval(self, splt):
        ...
        if has_labels:
            ...
            if self.is_classif:
                scores["%s_acc" % prefix] = 100.0 * (pred == gold).sum() / len(pred)
                scores["%s_f1" % prefix] = 100.0 * f1_score(
                    gold,
                    pred,
                    average="binary" if params.out_features == 2 else "micro",
                )
                scores["%s_mc" % prefix] = 100.0 * matthews_corrcoef(gold, pred)
            else:
                scores["%s_prs" % prefix] = 100.0 * pearsonr(pred, gold)[0]
                scores["%s_spr" % prefix] = 100.0 * spearmanr(pred, gold)[0]
    ...
    def load_data(self, task):
        ...
        for splt in ["train", "valid", "test"]:
            ...
            data2 = (
                load_binarized(os.path.join(dpath, "%s.s2.pth" % splt), params)
                if self.n_sent == 2
                else None
            )
            ...
            if self.n_sent == 1:
                data[splt]["x"] = Dataset(
                    data1["sentences"], data1["positions"], params
                )
            else:
            ...
            if splt != "test" or task in ["MRPC"]:
                ...
                # QQP
                    ...
                    UNK_LABEL = 0
                    lab2id = {
                        x: i for i, x in enumerate(sorted(set(lines) - set([""])))
                    }
