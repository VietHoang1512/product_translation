...
def mlp(sizes, bias=True, batchnorm=True, groups=1):
    ...
    for i, (dim_in, dim_out) in enumerate(pairs):
        if groups == 1 or i == 0:
            ...
            layers.append(
                GroupedLinear(
                    groups * dim_in, groups * dim_out, bias=bias, groups=groups
                )
            )
        if batchnorm:
...
def convs(
    channel_sizes, kernel_sizes, bias=True, batchnorm=True, residual=False, groups=1
):
    """
    ...
    pairs = [
        (channel_sizes[i], channel_sizes[i + 1]) for i in range(len(channel_sizes) - 1)
    ]
    layers = []
...
class GroupedLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=True, groups=1):
        ...
        self.layer = nn.Conv1d(
            in_features, out_features, bias=bias, kernel_size=1, groups=groups
        )
    ...
    def __init__(
        ...
        if input_channels == output_channels:
            ...
            self.residual = nn.Conv2d(
                input_channels, output_channels, (1, 1), bias=False, groups=groups
            )
...
class QueryIdentity(nn.Module):
    def __init__(self, input_dim, heads, shuffle_hidden):
        ...
        if shuffle_hidden:
            self.slices = {
                head_id: get_slices(input_dim, head_id) for head_id in range(heads)
            }
    ...
    def forward(self, input):
        ...
        if self.heads == 1:
            ...
            query = torch.cat(
                ...
                1,
            ).view(bs * self.heads, self.input_dim)
...
class QueryMLP(nn.Module):
    def __init__(
        ...
        if self.grouped_conv:
            self.query_mlps = mlp(
                sizes, bias=bias, batchnorm=batchnorm, groups=self.groups
            )
...
class QueryConv(nn.Module):
    def __init__(
        ...
        assert sizes[-1] == (k_dim // 2) if multi_query_net else (heads * k_dim)
        assert self.grouped_conv is False or len(sizes) > 2
        assert len(sizes) == len(kernel_sizes) + 1 >= 2 and all(
            ks % 2 == 1 for ks in kernel_sizes
        )
    ...
    def forward(self, input):
        ...
        query = (
            query.transpose(1, 3).contiguous().view(bs * w * h * self.heads, self.k_dim)
        )
        return query
