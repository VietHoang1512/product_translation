...
class HashingMemory(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, params):
        ...
        assert (
            self.modulo_size == -1
            and self.size == self.n_indices
            or self.n_indices > self.size == self.modulo_size >= 1
        )
        ...
        assert (
            self.use_different_keys is False
            or self.heads >= 2
            or self.product_quantization
        )
        assert (
            self.multi_query_net is False
            or self.heads >= 2
            or self.product_quantization
        )
        assert (
            self.shuffle_query is False
            or self.heads > 1
            and params.mem_query_layer_sizes == ""
        )
        ...
        self.values = nn.EmbeddingBag(
            self.size, self.v_dim, mode="sum", sparse=params.mem_sparse
        )
        ...
        if len(params.mem_query_layer_sizes) == 0:
            ...
            self.query_proj = QueryIdentity(
                self.input_dim, self.heads, self.shuffle_query
            )
        ...
        if len(params.mem_query_layer_sizes) > 0:
            ...
            l_sizes[-1] = (
                (self.k_dim // 2) if self.multi_query_net else (self.heads * self.k_dim)
            )
    ...
    def forward(self, input):
        ...
        output = self.values(
            indices, per_sample_weights=scores.to(self.values.weight.data)
        ).to(
            scores
        )  # (bs, v_dim)
        output = F.dropout(
        ...
        if self.input2d:
            output = output.view(
                n_images, width, height, self.v_dim
            )  # (n_images, width, height, v_dim)
        ...
        if not self.training and HashingMemory.EVAL_MEMORY:
            ...
            self.last_scores = (
                scores.view(bs, self.heads, self.knn).detach().cpu().float()
            )
    ...
    def register_args(parser):
        ...
        parser.add_argument(
            "--mem_k_dim", type=int, default=256, help="Memory keys dimension"
        )
        parser.add_argument(
            "--mem_v_dim",
            type=int,
        ...
        parser.add_argument(
            "--mem_heads", type=int, default=4, help="Number of memory reading heads"
        )
        parser.add_argument(
            "--mem_knn",
            type=int,
        ...
        parser.add_argument(
            "--mem_n_keys", type=int, default=512, help="Number of keys"
        )
        parser.add_argument(
            "--mem_keys_normalized_init",
            type=bool_flag,
            default=False,
            help="Normalize keys at initialization",
        )
        parser.add_argument(
            "--mem_keys_learn", type=bool_flag, default=True, help="Learn keys"
        )
        parser.add_argument(
            "--mem_use_different_keys",
            type=bool_flag,
            default=True,
        ...
        parser.add_argument(
            "--mem_query_bias", type=bool_flag, default=True, help="Query MLP bias"
        )
        parser.add_argument(
            "--mem_query_batchnorm",
            type=bool_flag,
        ...
        # dropout
        parser.add_argument(
            "--mem_input_dropout", type=float, default=0, help="Input dropout"
        )
        parser.add_argument(
            "--mem_query_dropout", type=float, default=0, help="Query dropout"
        )
        parser.add_argument(
            "--mem_value_dropout", type=float, default=0, help="Value dropout"
        )
    ...
    def check_params(params):
        ...
        params.mem_values_optimizer = (
            params.optimizer
            if params.mem_values_optimizer == ""
            else params.mem_values_optimizer
        )
        params.mem_values_optimizer = (
            params.mem_values_optimizer.replace("adam", "sparseadam")
            if params.mem_sparse
            else params.mem_values_optimizer
        )
        ...
        assert (
            not params.mem_use_different_keys
            or params.mem_heads >= 2
            or params.mem_product_quantization
        )
        assert (
            not params.mem_multi_query_net
            or params.mem_heads >= 2
            or params.mem_product_quantization
        )
        ...
        assert (
            not params.mem_shuffle_query
            or params.mem_heads > 1
            and params.mem_query_layer_sizes == ""
        )
        ...
        if params.mem_query_kernel_sizes == "":
            ...
            assert (
                len(params.mem_query_kernel_sizes)
                == len(params.mem_query_layer_sizes) - 1
                >= 1
            )
        ...
        assert not (
            params.mem_score_normalize
            and params.mem_score_softmax
...
class HashingMemoryFlat(HashingMemory):
    ...
    def init_keys(self):
        ...
        if self.keys_type in ["gaussian", "uniform"]:
            init = (
                get_gaussian_keys if self.keys_type == "gaussian" else get_uniform_keys
            )
            if self.use_different_keys:
                ...
                keys = torch.from_numpy(
                    init(self.n_indices, self.k_dim, self.keys_normalized_init, seed=0)
                )
    ...
    def _get_indices(self, query, knn, keys):
        ...
        # optionally normalize queries
        if self.normalize_query:
            query = query / query.norm(2, 1, keepdim=True).expand_as(
                query
            )  # (bs, kdim)

        # compute scores with indices
        scores = F.linear(query, keys, bias=None)  # (bs, n_keys)
        scores, indices = scores.topk(
    ...
    def get_indices(self, query, knn):
        ...
        if self.use_different_keys is False:
            ...
            indices = torch.cat([idx.unsqueeze(1) for _, idx in outputs], 1).view(
                bs, knn
            )
            return scores, indices
...
class HashingMemoryProduct(HashingMemory):
    ...
    def _get_indices(self, query, knn, keys1, keys2):
        ...
        with torch.no_grad():
            ...
            scores1, indices1 = get_knn_faiss(
                keys1.float(), q1.float(), knn, distance="dot_product"
            )  # (bs, knn) ** 2
            scores2, indices2 = get_knn_faiss(
                keys2.float(), q2.float(), knn, distance="dot_product"
            )  # (bs, knn) ** 2
            ...
            all_indices = (
                concat_indices[:, :, 0] * n_keys + concat_indices[:, :, 1]
            )  # (bs, knn ** 2)

            _scores, best_indices = torch.topk(
                all_scores, k=knn, dim=1, largest=True, sorted=True
            )  # (bs, knn)
